...
function initGrid() {
    ...
    $("#grid").jqGrid({
        url: "../basedata/inventory.aspx?action=list&isDelete=2",
        datatype: "json",
        width: a.w,
        height: a.h,
        altRows: !0,
...
function initEvent() {
    ...
    function() {
        if (Business.verifyRight("INVENTORY_EXPORT")) {
            ...
            $(this).attr("href", "/basedata/inventory.aspx?action=exporter&isDelete=2&skey=" + a + "&assistId=" + b)
        }
    }),
    ...
    $("#grid").on("click", ".operating .ui-icon-pic",
    function(a) {
        a.preventDefault();
        var b = $(this).parent().data("id"),
        c = "商品图片";
       /* $.dialog({
            content: "url:../settings/fileUpload.aspx",
            data: {
                title: c,
                id: b,
                callback: function() {}
...
handle = {
    operate: function(a, b) {
        ...
        var e = 1020,
        f = parent.SYSTEM.enableStorage ? 480 : 370;
        f = parent.SYSTEM.enableAssistingProp ? f: f - 100,
        f = parent.SYSTEM.ISSERNUM ? f + 40 : f,
        $.dialog({
            title: c,
    ...
    del: function(a) {
        $.dialog.confirm("删除的商品将不能恢复，请确认是否删除？",
        function() {
            Public.ajaxPost("../basedata/inventory.aspx?action=delete", {
                id: a
            },
            function(b) {
                if (b && 200 == b.status) {
    ...
    setStatus: function(a, b) {
        a && Public.ajaxPost("../basedata/inventory.aspx?action=disable", {
            invIds: a,
            disable: Number(b)
        },
        function(c) {
    ...
    setStatuses: function(a, b) {
        if (a && 0 != a.length) {
            ...
            d = c.join();
            Public.ajaxPost("../basedata/inventory.aspx?action=disable", {
                invIds: d,
                disable: Number(b)
            },
            function(c) {
