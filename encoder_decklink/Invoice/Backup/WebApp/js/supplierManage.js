function init() {
    void 0 !== cRowId ? Public.ajaxPost("../basedata/contact.aspx?action=get", {
        id: cRowId
    },
    function (a) {
    ...
    $_form.validator({
        rules: {
            ...
            unique: function(a) {
                var b = $(a).val();
                return $.ajax({
                    url: "../basedata/contact.aspx?action=checkName",
                    type: "get",
                    data: "name=" + b,
                    dataType: "json",
                    success: function(a) {
            ...
            myRemote: function(a, b, c) {
                return c.old.value === a.value || $(a).data("tip") === !1 && a.value.length > 1 ? !0 : $.ajax({
                    url: "../basedata/contact.aspx?action=getNextNo&type=-10",
                    type: "post",
                    data: "skey=" + a.value,
                    dataType: "json",
                    success: function(b) {
        ...
        valid: function() {
            var a = $.trim($("#name").val());
            Public.ajaxPost("../basedata/contact.aspx?action=checkName", {
                name: a,
                id: cRowId
            },
            function(b) { - 1 == b.status ? parent.$.dialog.confirm('供应商名称 "' + a + '" 已经存在！是否继续？',
                function() {
                    postCustomerData()
                },
...
function postCustomerData() {
    ...
    //,c = b.firstLink || {};
    //delete b.firstLink,
    Public.ajaxPost("../basedata/contact.aspx?action=" + ("add" == oper ? "add": "update"), b,
    function (d) {
        if (200 == d.status) {
            if (parent.parent.Public.tips({
                ...
                d = b,
                d.id = e,
                /*d.supplierType = d.cCategoryName,
                d.contacter = c.linkName || "",
...
function getCustomerData() {
    ...
    c = {
        ...
        name: $.trim($("#name").val()),
        contacter: $.trim($("#contacter").val()),
        mobile: $.trim($("#mobile").val()),
        telephone: $.trim($("#telephone").val()),
        linkIm: $.trim($("#linkIm").val()),
        ...
        receivable: $.trim($("#receivable").val()),
        preReceived: $.trim($("#preReceived").val()),
        note: $("#note").val() == $("#note")[0].defaultValue ? "" : $("#note").val()
    };
    return c//c.firstLink = a.firstLink,
    
}
/*
function getEntriesData() {
for (var a = {},
b = [], c = $grid.jqGrid("getDataIDs"), d = !1, e = 0, f = c.length; f > e; e++) {
var g, h = c[e],
...
g.province = j.province,
g.city = j.city,
g.county = j.county,
g.address = j.address,
g.id = "edit" == oper && -1 != $.inArray(h, linksIds) ? h: 0,
"是" == i.first && (d = !0, a.firstLink = g),
b.push(g)
}
return ! d && b.length > 0 && (b[0].linkFirst = 1, a.firstLink = b[0]),
a.entriesData = b,
a
}*/
function getTempData(a) {
    ...
    return c.supplierType = categoryData[c.cCategory] && categoryData[c.cCategory].name || "",
    c.firstLink = b,
    c
}
function initField() {
    if ($("#note").placeholder(), "edit" == oper) {
        if ($("#number").val(rowData.number), $("#name").val(rowData.name), $("#contacter").val(rowData.contacter),
             $("#mobile").val(rowData.mobile), $("#telephone").val(rowData.telephone), $("#linkIm").val(rowData.linkIm),
             $("#receivable").val(rowData.receivable), $("#preReceived").val(rowData.preReceived), $("#address").val(rowData.address))
        //$("#category").data("defItem", ["id", rowData.cCategory + ""]), 
        /*rowData.beginDate) {
        var a = new Date(Number(rowData.beginDate)),
        b = a.getFullYear(),
        c = 1 * a.getMonth() + 1,
        d = a.getDate();
        $("#date").val(b + "-" + c + "-" + d)
    ...
    levelCombo = $("#supplierLevel").combo({
        data: [{
            id: 0,
            name: "零售供应商"
        },
...
function initEvent() {
    ...
    return c.SYSTEM.categoryInfo = c.SYSTEM.categoryInfo || {},
    c.SYSTEM.categoryInfo[a] = b.data.items,
    b.data.items.unshift({
    id: 0,
    name: "（空）"
    }),
    b.data.items
    ...
    */
    ...
    $(".grid-wrap").on("click", ".ui-icon-ellipsis",
    function(a) {
        a.preventDefault();
        var b = $(this).siblings(),
        c = $(this).closest("tr"),
        ...
        parent.$.dialog({
            title: "联系地址",
            content: "url:../settings/addressManage.aspx",
            data: {
                rowData: d,
    ...
    function(a) {
        var b = a.target || a.srcElement; ! $(b).closest("#grid").length > 0 && cancleGridEdit()
    }),
...
function addCategory() {
    Business.verifyRight("BUTYPE_ADD") && parent.$.dialog({
        title: "新增供应商类别",
        content: "url:/settings/supplier-category-manage.aspx",
        data: {
            oper: "add",
            callback: function(a, b, c) {
                categoryCombo.loadData("../basedata/assist.aspx?action=list&typeNumber=suppliertype", ["id", a.id]),
                c && c.api.close()
            }
        },
        width: 400,
...
/*
function initGrid(a) {
    if (a || (a = []), a.length < 3) for (var b = 3 - a.length,
    c = 0; b > c; c++) a.push({});
    a.push({}),
    $grid.jqGrid({
        data: a,
...
function isFirstFormate(a) {
    return a = "boolean" == typeof a ? a ? "1": "0": a,
    "1" === a ? "是": "&#160;"
}
