...
function(a) {
    function b() {
        ...
        h("#filter-fromDate").attr("disabled", "disabled"),
        h("#filter-toDate").datepicker();
        var a = Public.urlParam();
        chkboxes = h("#chk-wrap").cssCheckbox(),
        i.enableAssistingProp || h("#chk-wrap").hide(),
        a = {
        ...
        "1" === a.showSku && h('#chk-wrap input[name="showSku"]').attr("checked", !0),
        h("#filter-fromDate").val(a.beginDate || ""),
        h("#filter-toDate").val(a.endDate || ""),
        l = Public.categoryTree(h("#filterCat"), {
            width: 200
        })
    }
    function c() {
        h("#refresh").click(function(a) {
            ...
            for (var e = 0,
            f = chkVals.length; f > e; e++) k[chkVals[e]] = 1;
            var g = h.dialog.tips("正在查询，请稍候...", 1e3, "loading.gif", !0);
            Public.ajaxGet("/report/invBalance.aspx?action=detail", k,
            function(a) {
        ...
        k.search && h("#refresh").trigger("click"),
        h("#btn-print").click(function(a) {
            a.preventDefault(),
            Business.verifyRight("InvBalanceReport_PRINT") && window.print()
        }),
        h("#btn-export").click(function(a) {
            if (a.preventDefault(), Business.verifyRight("InvBalanceReport_EXPORT")) {
                var b = {};
                for (var c in k) k[c] && (b[c] = k[c]);
                Business.getFile("/report/invBalance.aspx?action=exporter", b)
            }
        })
    }
    function d(a) {
    ...
    function e() {
        ...
        b = g(),
        c = h("#grid");
        m > Math.floor(b / 31).toFixed(0) ? c.jqGrid("setGridHeight", b) : c.jqGrid("setGridHeight", "auto"),
        c.jqGrid("setGridWidth", a, !1)
    }
    function f() {
