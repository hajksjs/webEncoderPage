function initFilter() {
    ...
    Public.dateCheck(),
    $(document).on("click", "#ui-datepicker-div,.ui-datepicker-header",
    function(a) {
        a.stopPropagation()
    }),
...
function initField() {
    ...
    c = "";
    a && b ? c = "「您已选择了<b>" + a.length + "</b>个客户，<b>" + b.length + "</b>个商品进行查询」": a ? c = "「您已选择了<b>" + a.length + "</b>个客户进行查询」": b && (c = "「您已选择了<b>" + b.length + "</b>个商品进行查询」"),
    $("#cur-search-tip").html(c)
}
function initEvent() {
    $("#refresh").on("click",
    function(a) {
    ...
    $("#btn-export").click(function(a) {
        if (a.preventDefault(), Business.verifyRight("PUREOORTDETAIL_EXPORT")) {
            ...
            Business.getFile("/report/puDetail.aspx?action=detailExporter", b)
        }
    }),
    $(".grid-wrap").on("click", ".link",
    function(a) {
        if (a.preventDefault(), Business.verifyRight("PU_QUERY")) {
            ...
            parent.tab.addTabItem({
                ...
                text: "购货单",
                url: "/purchase/purchase.aspx?id=" + b
            }),
...
function reloadReport() {
    ...
    window.location = "/report/purchaseDetail.apsx?action=detail" + a
}
