	 ...
	 //require("datepicker");
		...
		*/ 
		...
		Public.getDefaultPage = function(){
			...
			do{
				if (/default.aspx/.test(win.location.href)) {
					return win;
				}
				win = win.parent;
				i--;
		...
		Business.getFile = function(url, args, isNewWinOpen){
			...
			var downloadForm = defaultPage.$('form#downloadForm');
			if (downloadForm.length == 0) {
				downloadForm = defaultPage.$('<form method="post" />').attr('id', 'downloadForm').hide().appendTo('body');
			} else {
				downloadForm.empty();
			}
			...
			opts = $.extend(true, {
				data: function(){
					if(defaultPage.SYSTEM.customerInfo) {
						...
						return '/basedata/contact.aspx?action=list$simple=1';
					}
				}
			}, opts);
			...
			opts = $.extend(true, {
				data: function(){
					if(defaultPage.SYSTEM.salesInfo) {
						...
						return '/basedata/employee.aspx?action=list';
					}
				}
			}, opts);
		...
		Business.salesCombo = function($_obj, opts){
			...
			$('#quickAddSales').on('click', function(e){
				...
				$.dialog({
					title : '新增职员',
					content : 'url:/settings/staff-manage.aspx',
					data: {oper: 'add', callback: function(data, oper, dialogWin){
						//parent.getCustomer();
						//_self.customerCombo.selectByValue(data.id, false);
						salesCombo.loadData('/basedata/employee.aspx?action=list', ['id', data.id]);
						dialogWin && dialogWin.api.close();
					}},
					width : 400,
					height : 160,
			...
			opts = $.extend(true, {
				data: function(){
					if(defaultPage.SYSTEM.supplierInfo) {
						...
						return '/basedata/contact.aspx?action=listt&simple=1&type=10';
					}
				}
			}, opts);
		...
		Business.serNumManage = function(opts){
			...
			var params = {
				...
				content: 'url:/settings/serNumBatch.aspx',
				data: {
					invId: data.id,
					creatable:creatable,
			...
			if(enableStorage){
				...
				params.content = 'url:/settings/serNumList.aspx';
		...
		Business.billSkuManage = function(parentTr , data, isSkuSingle){
			...
			$.dialog({
				...
				content: 'url:http://'+defaultPage.location.hostname+'/settings/assistingProp-batch.aspx',
				data: {
					//skey:_self.skey,
			...
			var opts = $.extend(true, {
				...
				callback: {
					...
					incrementalSearch: function(pos, callback){
						var _self = this;
						var query = $_obj.val()
						Public.ajaxGet('/basedata/inventory.aspx?action=list', { rows: 20, skey: query }, function(data){
			...
			var opts = $.extend(true, {
				data: function(){
					if(defaultPage.SYSTEM.categoryInfo && defaultPage.SYSTEM.categoryInfo[typeNumber]) {
						...
						return '../basedata/assist.aspx?action=list&isDelete=2&typeNumber='+typeNumber;
					}
				},
				ajaxOptions: {
					formatData: function(data){
			...
			opts = $.extend(true, {
				data: function(){
					if(defaultPage.SYSTEM.storageInfo) {
						...
						return '/basedata/invlocation.aspx?action=list&isDelete=2';
					}
				}
			}, opts);
		...
		Business.storageCombo = function($_obj, opts){
			...
			if ($_obj.length == 0) { return };
			var opts = $.extend(true, {
					//data: parent.SYSTEM.storageInfo/*'/basedata/invlocation.aspx?action=list&isEnable=1'*/,
					data: function(){
			...
			if(type !== 'assemble') {
				$('#customer').on('click', '.ui-icon-ellipsis', function(e){	
					...
					if(lable === '供应商' || lable === '购货单位') {
						var content = 'url:/settings/select-customer.aspx?type=10';
					} else {
						var content = 'url:/settings/select-customer.aspx';
					}
				...
				$('.grid-wrap').on('click', '.ui-icon-ellipsis', function(e){
					...
					$.dialog({
							...
							height: 510,
							title: '选择商品',
							content: 'url:http://'+defaultPage.location.hostname+'/settings/goods-batch.aspx',
							data: {
								skuMult: skuMult,
								skey:_self.skey,
		...
		Business.filterSaler = function(){
			...
			$('#filter-saler .ui-icon-ellipsis').on('click', function(){
				...
				$.dialog({
					...
					height: 500,
					title: '选择销售员',
					content: 'url:/settings/saler-batch.aspx',
					data:{isDelete:2},
					lock: true,
					ok: function(){
						Business.setFilterData(this.content, $input);
			...
			Public.mod_PageConfig = (function(mod) {
				...
				function _config(){
					for(var gridId in conf.grids){
						if(typeof conf.grids[gridId] != 'function' && conf.grids[gridId].isReg){
							var $grid = $('#' + gridId);
							var content = [
								'<div class="content">',
									'<div class="grid-wrap">',
							...
							var dataArr = conf.grids[gridId].colModel;
							var _dataArr=[];
				...
				function _gridReg(gridId , defColModel){//表格一定要注册默认配置信息
					if(!defColModel){
						return;
					}
				...
				function _modelClone(c1, c2){
					return $.extend(true, c1, {//开放修改的列属性
								label: c2['label']//列名
				...
				function _eventReg(){
					...
					$(window).on('unload',function(){
						if(conf && Date.parse(conf.modifyTime) != Date.parse(conf.curTime)){
							conf.modifyTime = conf.curTime;

							$.post('/basedata/userSetting.aspx?action=update',{
								key : mod.pageId,
								value : _formatPostData()
							},function(data){
								//更新后的回调
							})
						}
