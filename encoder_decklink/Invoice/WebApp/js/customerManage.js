...
function initValidator() {
    $_form.validator({
        ...
        valid: function() {
            ...
            function(b) { - 1 == b.status ? parent.$.dialog.confirm('客户名称 "' + a + '" 已经存在！是否继续？',
                function() {
                    postCustomerData()
                },
...
function postCustomerData() {
    ...
    //,c = b.firstLink || {};
    //delete b.firstLink,
    Public.ajaxPost("../basedata/contact.aspx?type=1&action=" + ("add" == oper ? "add" : "update"), b,
    function (d) {
        if (200 == d.status) {
            if (parent.parent.Public.tips({
                ...
                d = b,
                d.id = e,
                /*d.customerType = d.cCategoryName,
...
function getTempData(a) {
    ...
    return c.customerType = categoryData[c.cCategory] && categoryData[c.cCategory].name || "",
    c.firstLink = b,
    c
}
function initField() {
...
function addCategory() {
    Business.verifyRight("BUTYPE_ADD") && parent.$.dialog({
        title: "新增客户类别",
        content: "url:/settings/customer-category-manage.aspx",
        data: {
            oper: "add",
            callback: function(a, b, c) {
                categoryCombo.loadData("../basedata/assist.aspx?action=list&typeNumber=customertype", ["id", a.id]),
                c && c.api.close()
            }
        },
        width: 400,
