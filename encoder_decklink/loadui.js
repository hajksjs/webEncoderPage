...
function loadoutputset()
{
  var urloutput="setting/output.php";
	$.ajax({  
  type: "post",  
  url: urloutput,  
  dataType: "json",  
  ...
  success: function(msg){
   $.each(msg,function(name,value) {       	
        	var optiona = "<option value='" + name + "'>" + value + "</option>";
            $("#selectoutputset").append(optiona);
        }); 
		var count=$("#selectvediolevelset option").length; 
		$("#selectoutputset").val("1");
  }     
                ...
                //alert("msg[istart].input_name != undefined" + msg[istart].input_name);
						...
						else if(msg[istart].video_encpara == 1)
						{
							if(msg[istart].bitrate != undefined)
							{
								...
								msg[istart].bitrate = msg[istart].bitrate/1000;
...
{
	var asiOutPut=$("#selectoutputset").find("option:selected").text();
	
//（输出部分ASI还是IP）--------------------------------------------------//	
	var paraOutPut;
	if($("#outputASI").prop("checked"))
	{
		if("output1" == asiOutPut)
		{
			paraOutPut = " -asisending 1 -y ";
			paraOutPut += $("#textfieldoutputnameset").val();//"asi://192.168.3.67:9008" ;
		}
		else if("output2" == asiOutPut)
		{
			paraOutPut = " -asisending 2 -y ";
			paraOutPut += $("#textfieldoutputnameset").val();//"asi://192.168.3.67:9008" ;
		}
	}
	else if($("#outputIP").prop("checked"))
	{
		paraOutPut = " -asisending 0 -y ";
		paraOutPut += $("#textfieldoutputnameset").val();
	}
	//alert(asiOutPut);
	//alert(paraOutPut);
	
	// if(document.getElementById("outputASI").checked)  
    // {  
        // alert("1");  
    // } 
	
	// var paraOutPut = $("[name='output']").filter(":checked"); 
	// paraOutPut = paraOutPut.attr("id");
	// if("outputASI" == paraOutPut)
	// {
		// paraOutPut = "asi://233.233.233.223:6666" ;
	// }
	// else if("outputIP" == paraOutPut)
	// {
		// paraOutPut = $("#textfieldoutputnameset").val();
	// }
	//alert(paraOutPut);
//-----------------------------------------------------------------------//	
	...
	var selectvediobitrateset = $("#inputvediobit").val()*1000;
	...
	//alert($("#selectdevice").find("option:selected").val());
	//alert($("#selectdevice").find("option:selected").text());
	$.ajax({  
		type: "post",  
		url: urlencoder,  
...
function statusphp(){	
	document.forms[0].action="status.php";
...
function setphp(){
	document.forms[0].action="set.php";
}
